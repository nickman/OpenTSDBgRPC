 ...
 * <p><code>net.opentsdb.grpc.server.streaming.client.AbstractStreamer</code></p>
 */
...
public abstract class AbstractStreamer<T, R> implements Streamer<T, R> {
	...
	private static final Cache<MethodDescriptor<?, ?>, Class<?>[]> TYPE_CACHE = 
	...
	protected final LongAdder inFlight = new LongAdder();
	...
	private Class<?>[] types() {
		...
		try {
			return TYPE_CACHE.get(md, new Callable<Class<?>[]>() {
				@Override
				public Class<?>[] call() throws Exception {
					Marshaller<T> rqm = md.getRequestMarshaller();
					if(rqm instanceof ReflectableMarshaller) {
						classes[0] = ((ReflectableMarshaller<T>)rqm).getMessageClass();
						LOG.info("Caching RequestType for {}: {}", md.getFullMethodName(), classes[0].getName());
					} else {
						classes[0] = Object.class;
						LOG.info("No RequestType for {}", md.getFullMethodName());
					}
					Marshaller<R> rsm = md.getResponseMarshaller();
					if(rsm instanceof ReflectableMarshaller) {
						classes[1] = ((ReflectableMarshaller<R>)rsm).getMessageClass();
						LOG.info("Caching ResponseType for {}: {}", md.getFullMethodName(), classes[1].getName());
					} else {
						classes[1] = Object.class;
						LOG.info("No ResponseType for {}", md.getFullMethodName());
					}					
					return classes;
				}				
			});
	...
	protected AbstractStreamer(StreamerBuilder<T,R> builder) {
		...
		Class<?>[] types = types();
		requestType = (Class<T>) types[0];
		responseType = (Class<R>) types[1];		
	...
	/**
	 * {@inheritDoc}
	 * @see net.opentsdb.grpc.server.streaming.client.Streamer#start()
	 */
	...
	/**
	 * {@inheritDoc}
	 * @see net.opentsdb.grpc.server.streaming.client.Streamer#printStats()
	 */
	@Override
